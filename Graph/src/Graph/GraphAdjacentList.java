...
public class GraphAdjacentList {
     ...
     **/
    public GraphAdjacentList(ArrayList<Vertice> vertices)
    {
	this.vertices = new HashMap<String, Vertice>();
	this.aristas = new HashMap<Integer, Arista>();
	...
	return new HashSet<Arista>(this.aristas.values());
	  ...
	  private void restablecerEstadaVista(Vertice node) {
	    	
	    	ArrayList<Vertice> vertices = node.getAdyacentes(); 
		...
		public void bfs(Vertice node) {
			restablecerEstadaVista(node);
			...
			while (!queue.isEmpty())
			{
				...
				Vertice element=queue.remove();
				
				List<Vertice> neighbours = element.getAdyacentes();
				for (int i = 0; i < neighbours.size(); i++) {
					Vertice n = neighbours.get(i);
		...
		// Iterative DFS using stack
			...
			public void dijkstra(Vertice node) {
				restablecerEstadaVista(node);
				...
				while(!pQueue.isEmpty()) {
					Vertice u = pQueue.poll(); 
					u.setVisitado(true);
					for (Vertice vertice : pQueue) {
						if (u.getAdyacentes().contains(vertice)) {
