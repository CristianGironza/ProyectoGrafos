...
public class GraphMatrix {
	...
	VertexMatrix vertexList[][];  //2d array of the vertices
	int amountVertices; //total vertices in maze
	int dimension; // dimension of maze
	private Random myRandGen; // random number generator
	private VertexMatrix startVertex; //start of maze
	private VertexMatrix endVertex; //end of maze
	...
	public GraphMatrix(int dimension_in) {
		vertexList = new VertexMatrix[dimension_in][dimension_in];
		...
		for (int i = 0; i < dimension_in; i++) {
			for (int j = 0; j < dimension_in; j++) {
				vertexList[i][j] = new VertexMatrix(i + r + j);
			}
	 ...
	 */
	public void setPath(){
		VertexMatrix current = vertexList[dimension-1][dimension-1];
		while (current != null){
			current.inPath = true;
			current = current.pi;
		}
	 ...
	 */
	public void DFS(VertexMatrix s) {
		int traverseO = 1;  
		Stack<VertexMatrix> pila = new Stack<>();
		pila.push(s);
		while (!pila.isEmpty() && !pila.peek().equals(endVertex)) { //while we aren't at the end
			VertexMatrix u = pila.pop();
			for (int i = 0; i < u.neighbors.length; i++) {
				VertexMatrix v = u.neighbors[i];
	 ...
	 * @param s - the starting vertex - the root
	 */
	public void BFS(VertexMatrix s) {
		int traverseO = 1;  
		Queue<VertexMatrix> q = new LinkedList<>();
		q.add(s);
		while (!q.isEmpty() && !q.peek().equals(endVertex)) { //while we aren't at the end
			VertexMatrix u = q.remove();
			for (int i = 0; i < u.neighbors.length; i++) {
				VertexMatrix v = u.neighbors[i];
				int direction = u.vertexRelationship(v);
				if ((u.walls[direction] == 0) && v != null && v.color == 0) { //if the wall is broken and it's not null and it hasn't been visited
	...
	public void populateGraph() {
		for (int i = 0; i < dimension; i++) {
			for (int j = 0; j < dimension; j++) {
				VertexMatrix mine = vertexList[i][j];
	...
	void generateMaze() {
		...
		Stack<VertexMatrix> cellStack = new Stack<>();
		int totalCells = amountVertices;
		VertexMatrix currentCell = vertexList[0][0];
		int visitedCells = 1;
		while (visitedCells < totalCells) {
			ArrayList<VertexMatrix> neighborsIntact = new ArrayList<VertexMatrix>();
			for (int i = 0; i < currentCell.neighbors.length; i++) {
				VertexMatrix neighbor = currentCell.neighbors[i];
				if (neighbor != null) {
			...
			if (neighborsIntact.size() != 0) {
				...
				VertexMatrix knockDown = neighborsIntact.get(rand);
