		...
		for (int i = 0; i < filas; i++) {
			for (int j = 0; j < columnas; j++) {
				...
				//if (i != filas - 1) {
					...
					if(grafo!=null){
						if((i+1<filas) && actual.getVecinos().contains(new Vertice( (i+1)*columnas+j  ))){
							abajo.setBackground(Color.white);
							casillas[i][j].setVecinoInf(true);
						}
						if((i-1>=0) && actual.getVecinos().contains(new Vertice( (i-1)*columnas+j  ))){
				...
				//if (j != columnas - 1) {
					...
					if(grafo!=null){
						if((j+1<columnas) && actual.getVecinos().contains(new Vertice( (i*columnas)+j+1 ))){
							derecha.setBackground(Color.white);
							casillas[i][j].setVecinoDer(true);
						}
						if((j-1>=0) && actual.getVecinos().contains(new Vertice( (i*columnas)+j-1 ))){
