...
public class GrafoMatriz<T> extends Grafo implements IGrafoMatriz<T>  {
	
	private ArrayList<ArrayList<Arista<T>>> aristas;

	private ArrayList<Vertice<T>> vertices;

	public GrafoMatriz(ArrayList<Vertice> nodos) {
	...
	@Override
	public IVertice<T> crearVertice(T vertice, T[] adyacentes, String[] nombres, double[] pesos, int direccion) {
	...
	@Override
	public IVertice<T> crearVertice(T vertice, T[] adyacentes, String[] nombres, int direccion) {

		return null;
	}

	@Override
	public AristaMatriz<T> dijsktraParcial(T ini, T f) {
		Vertice<T> fin = get(f);
		if (fin != null) {
			int index = -1;
			...
			if (index >= 0) {
				...
				AristasMatriz<T> ca = dijsktra(inicio);
				AristaMatriz<T> salida = ca.getRutas()[index];
				return salida;
			} else {
				return null;
			}
	...
	public AristaMatriz<T>[][] floydWarshall() {
		int tam = vertices.size();
		AristaMatriz<T>[][] caminos = new AristaMatriz[tam][tam];
		for (int i = 0; i < caminos.length; i++) {
			for (int j = 0; j < caminos.length; j++) {
				...
				AristaMatriz<T> nuevo = new AristaMatriz<>(c, pes);
				caminos[i][j] = nuevo;
			}
		}
		for (int i = 0; i < tam; i++) {
					...
					for (int j2 = 0; j2 < caminos.length; j2++) {
						if (j2 != i && j != j2) {
							...
							if (pesoFp != Double.MAX_VALUE) {
								...
								if (suma < peso) {
									AristaMatriz<T> f = caminos[j][i];
									ArrayList<Vertice<T>> p = f.getRuta();
									AristaMatriz<T> s = caminos[i][j2];
									...
									nueva.addAll(p);
									nueva.addAll(aux);
									AristaMatriz<T> nuevos = new AristaMatriz<>(nueva, suma);
									caminos[j][j2] = nuevos;
								}
							}
	...
	public AristasMatriz<T> dijsktra(Vertice<T> inicio) {
		if(vertices != null){
			if (inicio != null) {
				int tam = vertices.size();
				double[] pesos = new double[tam];
				@SuppressWarnings("unchecked")
				AristaMatriz<T>[] caminos = new AristaMatriz[tam];
				int index = 0;
				boolean mar[] = new boolean[tam];
				for (int i = 0; i < pesos.length; i++) {
					pesos[i] = Double.MAX_VALUE;
					caminos[i] = new AristaMatriz<>();
				...
				AristasMatriz<T> n = new AristasMatriz<>(caminos, pesos);
				return n;
			} else {
				return null;
			}
