...
public class GrafoLista<T> implements IGrafoLista<T> {
	...
	public VerticeLista<T> crearVertice(T element, T[] adyacents, String[] names, double[] weights, int address) {
		if (adyacents != null) {
			...
			for (int i = 0; i < adyacents.length; i++) {
				...
				Arista<T> nuevaA = new Arista<>(weights[i], names[i], no);
				if (address == NO_DIRIGIDO) {
					no.agregarArista(nueva, weights[i], names[i]);
					if (vertices == null)
	...
	public VerticeLista<T> crearVertice(T element, T[] adyacents, String[] names, int address) {
		if (adyacents != null) {
			VerticeLista<T> nueva = darVertice(element) == null ? new VerticeLista<>(element)
					: (VerticeLista<T>) darVertice(element);
			ArrayList<Arista<T>> aristas = new ArrayList<>();
	...
	public AristaLista<T> dijsktraParcial(T ini, T f) {
		...
		if (fin != null) {
			...
			if (index >= 0) {
				...
				AristasLista<T> ca = dijsktra(inicio);
	...
	@Override
	public AristasLista<T> dijsktra(VerticeLista<T> inicio) {
		ArrayList<VerticeLista<T>> vertices = getVertices();
		...
		if (vertices != null) {
			if (inicio != null) {
				...
				AristaLista<T>[] caminos = new AristaLista[tam];
				int index = 0;
				boolean mar[] = new boolean[tam];
				for (int i = 0; i < pesos.length; i++) {
					pesos[i] = Double.MAX_VALUE;
					caminos[i] = new AristaLista<T>();
				...
				AristasLista<T> n = new AristasLista<>(caminos, pesos);
	...
	@Override
	public HashMap<T, AristasLista<T>> floydWarshall() {
		HashMap<T, AristasLista<T>> ret = new HashMap<>();
		ArrayList<VerticeLista<T>> vertices = getVertices();

		for (int i = 0; i < vertices.size(); i++) {
			AristasLista<T> cami = dijsktra(vertices.get(i));
